...
export const sortUpdatedPromociones = async (token, promociones) => {
  try {
    let dataToSend = new FormData();
    Object.keys(promociones).forEach((key) => {
      if (promociones[key]) {
        dataToSend.append(key, promociones[key]);
      }
    });
    let data = await fetch(`${API_URL}/sortUpdate/promociones`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${token}`,
...
export const sortUpdatedCombos = async (token, combos) => {
  try {
    let dataToSend = new FormData();
    Object.keys(combos).forEach((key) => {
      if (combos[key]) {
        dataToSend.append(key, combos[key]);
      }
    });
    let data = await fetch(`${API_URL}/sortUpdate/combos`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${token}`,
...
export const sortUpdatedBanner = async (token, banner) => {
  try {
    let dataToSend = new FormData();
    ...
    let data = await fetch(`${API_URL}/sortUpdate/banner`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${token}`,
